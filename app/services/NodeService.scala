...
import org.ergoplatform.appkit.{ErgoClient, InputBox, NetworkType, RestApiErgoClient}
import org.ergoplatform.restapi.client.{ApiClient, NodeApi, UtxoApi}
...
class NodeService() {
  ...
  val ergoClient: ErgoClient = RestApiErgoClient.create(Conf.activeNodeUrl, NetworkType.MAINNET, "", "https://api.ergoplatform.com/api/v1/")
  ...
  def getAcBox(): Json = {
    unspentBoxesFor(Conf.acScanID, minConf = 0).last
  }

  def getLpBox(): Json = {
    unspentBoxesFor(Conf.lpScanId, minConf = 0).head
  }

  def getBuyBackBoxes(): Seq[Json] = {
    unspentBoxesFor(Conf.buyBackScanId, minConf = -1)
  }
  ...
  def getUnspentBoxFromMempool(boxId: String): InputBox = {
    val nodeApi = new ApiClient(Conf.activeNodeUrl)
    val nodeService = nodeApi.createService(classOf[UtxoApi])
  ...
  def getTimestamp(): Long = {
    try {
      val info = Http(s"${Conf.activeNodeUrl}/info").headers(Seq[(String, String)](("Content-Type", "application/json"))).asString
      parse(info.body).getOrElse(Json.Null).hcursor.downField("currentTime").as[Long].getOrElse(0L)
    } catch {
      case _: Throwable => 0L
    }
  }
